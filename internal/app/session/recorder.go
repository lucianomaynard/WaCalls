...
// direções: áudio do agente (browser) + áudio do lead (peer). Portado do fork
// lucianomaynard/WaCalls para o ramo perfex (base: develop do JotaDev66/WaCalls).
//
...
// WriteAgent recebe o PCM capturado do microfone do agente (browser). Seguro com r nil
// (gravação desligada): &r.agent num ponteiro nil derrubaria o processo.
func (r *Recorder) WriteAgent(pcm []float32) {
	if r == nil {
		return
	}
	r.ingest(&r.agent, pcm)
}

// WritePeer recebe o PCM decodificado do lado do lead (peer). Seguro com r nil.
func (r *Recorder) WritePeer(pcm []float32) {
	if r == nil {
		return
	}
	r.ingest(&r.peer, pcm)
